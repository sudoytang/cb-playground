...
\author[Group 23]
{Irys Zhang\inst{1} \and Jiangchuan Yu\inst{1} \and Yushun Tang\inst{1}}
...
2b & + Finetune on clickbait       & \alert{5.216} & \alert{184} \\
3 & BART fine-tune                 & \multicolumn{2}{c}{\textit{in progress}} \\
\bottomrule
\end{tabular}
\end{table}
...
Two-stage pretrain $\rightarrow$ finetune reduces perplexity by \textbf{65\%} vs.\ direct training (184 vs.\ 533), confirming that general summarisation knowledge transfers effectively to clickbait generation.
\end{alertblock}
...
\begin{enumerate}
    ...
    \item Pretrain--finetune LSTM achieves \textbf{PPL 184}; BART expected to push further
\end{enumerate}
\end{block}
...
\begin{itemize}
    ...
    \item Complete BART experiment and compare fluency via \textbf{human evaluation}
